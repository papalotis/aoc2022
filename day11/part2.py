...
class Operation:
    ...
    def __call__(self, worry: int) -> int:
        ...
        return int(eval(f"{left} {self.operand} {right}"))
...
class MonkeyGame:
    ...
    def run_one_round(self) -> None:
        prod_of_mod_values = math.prod(
            monkey.modulo_test_value for monkey in self.monkeys
        )
        ...
        for monkey in self.monkeys:
            monkey.items = deque(x % prod_of_mod_values for x in monkey.items)
